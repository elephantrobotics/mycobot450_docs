...
**myStudio Pro** 是一款集多功能于一体的机器人编程控制软件，为用户提供可视化编程交互、快捷移动控制、拖动教学、机器人状态查询与配置等一站式解决方案。软件内主要集成四大功能模块：`myBlockly`、`快速移动`、`资源链接`、`设置`、`modbus`覆盖从编程到调试、从学习到部署的全流程需求。
...
模块为用户提供便捷的资源导航功能，集中展示常用外部链接入口，包括技术文档、官方联系方式等。用户无需手动查找，即可快速访问相关支持材料，提升使用与维护效率。
...
模块为用户提供便捷的MODBUS协议权限控制，通过MODBUS协议可实现设备的交互控制，启用后设备通过该模块可预览寄存器地址功能表或进行交互操作。
...
功能：通过MODBUS协议可实现设备的交互控制，启用设备后通过该模块可预览寄存器地址功能表或进行交互操作。
...
1、当捕获到机械臂运动时，会弹出捕获到的具体警告弹窗，弹窗主要由4部分组成，1：当前具体的异常错误内容；2：当前异常错误的解决方法，若当前异常可解决则会显示，反之则没有内容显示；3：当前可清除或恢复的异常错误的'修复按钮'，触发即自动对该异常进行修复处理，反之则没有按钮显示；4：当前异常确认按钮，如果你不想处理错误，可以该按钮忽略当前异常。
...
3、若异常可修复可点击修复按钮（1）进行异常修复操作，点击清理记录按钮（2）会对已解决的历史报警记录进行清除操作。
