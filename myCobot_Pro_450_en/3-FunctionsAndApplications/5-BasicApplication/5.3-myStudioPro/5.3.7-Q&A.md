...
**Q9:** Upgrading MyStudioPro from an Older Version to the Latest Version (v2.210.1212)
1. You can access the new version of MyStudioPro by flashing the [latest system image](../5.4-SystemUpdate.md).
2. Manual Upgrade via Installation Packages
    ...
    - Upload the downloaded software files to the robotic arm from your PC using `MobaXterm` or `Xftp`.

    - Connect to the robotic arm via SSH from your PC (Target path: `/root/`):
        ```shell
        ssh root@192.168.0.232
        ```

    - Execute the following commands to extract `api.tar.gz`:
        ```shell
    ...
    - Execute the following commands to extract `app.zip`:
        ```shell
    ...
    - Install MyCobotPro:
        ...
        ```
    - The latest version becomes available for use after restarting the robotic arm.
