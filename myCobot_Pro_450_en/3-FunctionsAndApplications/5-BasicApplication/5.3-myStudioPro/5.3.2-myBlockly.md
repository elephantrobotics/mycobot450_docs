...
What is **Fast Move**? Simply put, it's controlling the robot's movement quickly with just mouse clicks, without any programming.

> Note: myStudio Pro has two **Fast Move** control panels; their functions are largely the same.

**Step 1:** Click the **Fast Move** button to open the **Fast Move** panel and wait for the robot data to return.
...
Drag a `Set Coordinates` block to the workspace, and click the `Quick Fill` button on the block to fill in the data. Change the `Z` axis data to `300`.
...
Single-step execution refers to executing one block at a time. When there are many blocks in the workspace, I only want to execute one block to see the effect. This is where you can use the "Single Step" button.

> The "Single Step" button is disabled by default and cannot be clicked.
...
**Step 2:** Select the "Joint Setting" block. You will observe that the "Single Step" button has been unlocked (its color has changed to a darker shade).

<img src="../../../resources/3-FunctionsAndApplications/5.myBlockly/blockly/singleStep2.png" />

**Step 3:** Click the "Single Step" button. This will automatically open the run panel and begin executing the code. Simultaneously, observe the changes in the robotic arm's movement. (Single-step execution sets joint 1 to 30°)
...
Click the "Run Panel" button. When the Run Panel pops up, click "Run All," and immediately click the "Pause" button. The program will pause after executing the first instruction `mc.send_angle(1,0,20)`.
...
- If you click the `Step In` button, the program will execute the next instruction, `mc.send_angle(1,-90,20)`;
...
This chapter introduces how to use the drag-and-teach function in `blockly`.
...
This chapter introduces how to use `blockly` to control the Pro force control gripper connected to a robotic arm.
...
1: `Set Pro Force Control Gripper Angle`: Makes the gripper enter a specified state (open or closed) at a specified angle.
...
2: `Read Pro Force Control Gripper Angle`: Reads the current gripper angle value.
...
- Control the Pro force control gripper to reach a position with a value of `80`

- Wait 3 seconds

- Control the Pro force control gripper to reach a position with a value of `20`

- Wait 3 seconds

- Read the current angle value of the Pro force control gripper

- Wait 3 seconds

- Control the Pro force control gripper to reach a position with a value of `0`

**Note**:

If you cannot control the gripper from the above examples, please ensure that your Pro force control gripper communication method is `modbus` protocol control.
...
|6 | Clicking the "Overwrite Current Waypoint" button quickly overwrites the coordinate information of all selected waypoints with the current robot arm coordinates.
...
|9 | Clicking the "Add Waypoint" button records the current robot arm coordinates.
|10 | Clicking the "Generate Code" button quickly generates waypoint blocks in the Blockly program coding area.

Select a waypoint block in `blockly` and add it to the program coding area. Alternatively, use the `Generate Code` button in the waypoint list to add the generated waypoint block to the last block in the workspace.


<img src="../../../resources/3-FunctionsAndApplications/5-BasicApplication/5.3-myStudioPro/blockly/waypoint1.png" />


<img src="../../../resources/3-FunctionsAndApplications/5-BasicApplication/5.3-myStudioPro/blockly/waypoint2.png" />

The waypoint block associates data from the waypoint list. Select any waypoint in the list to associate.

<img src="../../../resources/3-FunctionsAndApplications/5-BasicApplication/5.3-myStudioPro/blockly/waypoint3.png" />

<img src="../../../resources/3-FunctionsAndApplications/5-BasicApplication/5.3-myStudioPro/blockly/waypoint4.png" />
...
<img src="../../../resources/3-FunctionsAndApplications/5-BasicApplication/5.3-myStudioPro/blockly/waypoint5.png" />
...
Function Description: By recording multiple track data (robotic arm joints), quickly control the robotic arm to reach each track point position. Open the track list page, click the "Record Track" button, the joints of the robotic arm will relax, manually drag the robotic arm to record the track, click "Stop Recording" and wait for the track to be saved. Once saved, the track recording is complete. Successfully recorded tracks will automatically generate a track type file in the "Test Space" (For file function related operations, [please click here](./5.5.13-file.md)).

<img src="../../../resources/3-FunctionsAndApplications/5-BasicApplication/5.3-myStudioPro/blockly/trajectory1.png" />
...
| 3 | Clicking the "Reproduce" button allows you to reproduce the trajectory.|
...
Then, select the trajectory block in `blockly` and add it to the program programming area. Clicking the drop-down menu of a trajectory block in the workspace allows you to select data from the current trajectory list page.

<img src="../../../resources/3-FunctionsAndApplications/5-BasicApplication/5.3-myStudioPro/blockly/trajectory2.png" />

<img src="../../../resources/3-FunctionsAndApplications/5-BasicApplication/5.3-myStudioPro/blockly/trajectory3.png" />

<img src="../../../resources/3-FunctionsAndApplications/5-BasicApplication/5.3-myStudioPro/blockly/trajectory4.png" />
After selection, open the Run panel and click Run All to reproduce the trajectory using the block-by-block method.

<img src="../../../resources/3-FunctionsAndApplications/5-BasicApplication/5.3-myStudioPro/blockly/trajectory5.png" />

<img src="../../../resources/3-FunctionsAndApplications/5-BasicApplication/5.3-myStudioPro/blockly/trajectory6.png" />
...
| 1 | Test File Space: Switching to this option will paginate and display all test files. |

| 2 | Production File Space: Switching to this option will paginate and display all production files. |
...
| 4 | Query condition input box. Currently, only fuzzy search by filename is supported. You can press Enter to search directly or click the function 5 button.
|
| 5 | Query button. Clicking this button allows you to perform a paginated search based on the input box content.
|
...
Click the `Restore` button to restore the file from the recycle bin to the test space.
...
You can choose to run the file infinitely or specify the number of runs (the following explains the looping process). After setting, it will run in a loop. First, it will prompt you twice for confirmation; click "OK" to start the loop.
...
[← Previous Page](./5.3.1-myStudioFirstUse.md) | [Next Page →](./5.3.3-quickmove.md)